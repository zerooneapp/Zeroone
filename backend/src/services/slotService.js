...
const normalizeToGrid = (time) => {
  ...
  // Snap to start of 15-minute grid (0, 15, 30, 45)
  if (minutes < 15) {
    m.minutes(0);
  } else if (minutes < 30) {
    m.minutes(15);
...
const findFirstAvailableStaff = async (vendorId, serviceIds, startTime, endTime, targetStaffId = null, excludeBookingId = null, excludeUserId = null) => {
  ...
  if (excludeUserId) {
    lockFilter.userId = { $ne: excludeUserId };
  }

  const slotLocks = await SlotLock.find(lockFilter);
