...
const AddWalkInModal = ({ isOpen, onClose, onRefresh }) => {
  const { servicesData: services, staffData: staff } = useVendorStore();
  ...
  // Data is now provided by the global vendorStore, pre-fetched at dashboard load.
                ...
                {services.map(s => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-3 py-2 rounded-lg text-[9px] font-black capitalize tracking-wider border transition-all ${isSelected ? 'bg-slate-50 dark:bg-blue-500/10 border-[#00246b] text-[#00246b] shadow-sm transform scale-105' : 'bg-white dark:bg-gray-900 border-slate-200/60 dark:border-gray-800 text-slate-400 dark:text-gray-500 shadow-sm'}`}
                    >
                      {s.name} - ₹{s.price}
                    </button>
                  );
              ...
              <div className="grid grid-cols-2 gap-2 p-1.5">
                {staff.map(st => (
                  <button
                    ...
                    className={`flex items-center gap-2.5 p-2 rounded-xl border transition-all ${formData.staffId === st._id ? 'bg-slate-50 dark:bg-blue-500/10 border-[#00246b] dark:border-blue-500 shadow-xl transform scale-105' : 'bg-white dark:bg-gray-900 border-slate-200/60 dark:border-gray-800 text-slate-400 dark:text-gray-500 shadow-sm'}`}
                  >
                    ...
                    <span className={`text-[9px] font-black capitalize truncate ${formData.staffId === st._id ? 'text-[#00246b] dark:text-white' : 'text-slate-400 dark:text-gray-500'}`}>{st.name}</span>
                  </button>
                ))}
              </div>
            </div>
