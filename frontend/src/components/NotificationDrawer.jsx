...
const NotificationDrawer = ({ isOpen, onClose }) => {
  ...
  const getIcon = (type) => {
    switch (type) {
      ...
      case 'BOOKING_RESCHEDULED': return <Clock3 size={16} className="text-primary dark:text-white" />;
      case 'ASSIGNMENT_RECEIVED': return <ShoppingBag size={16} className="text-primary dark:text-white" />;
      case 'LOW_BALANCE': return <CreditCard size={16} className="text-orange-500" />;
      default: return <Bell size={16} className="text-primary dark:text-white" />;
    }
  };

  // Show all notifications but allow visual distinction for unread ones
                ...
                activeNotifications.map((n) => (
                  <motion.div
                    ...
                    className={`p-3.5 rounded-2xl border transition-all relative group cursor-pointer ${
                      n.isRead 
                        ? 'bg-white/40 dark:bg-gray-800/20 border-transparent opacity-60' 
                        : 'bg-white/80 dark:bg-gray-800/60 backdrop-blur-xl border-white/60 dark:border-white/5 shadow-xl hover:shadow-2xl'
                    }`}
