...
import {
  ...
  Plus, History, TrendingUp, Eye, MousePointer2, AlertCircle, Trash2
} from 'lucide-react';
            ...
            <AnimatePresence mode="popLayout" initial={false}>
              {vendors.map((vendor) => (
                ...
                >
                  ...
                  <td className="px-3 py-3.5 w-[10%]">{getStatusBadge(vendor.status, vendor)}</td>
                  <td className="px-3 py-3.5 text-center w-[5%]">
      ...
      <AnimatePresence>
        {isDrawerOpen && selectedVendor && (
          <>
            ...
            >
              <div className="p-6 px-7 border-b border-slate-100 dark:border-gray-800/50 flex items-center justify-between bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl sticky top-0 z-50">
                <div className="flex items-center gap-5">
                  ...
                  <div className="leading-tight pt-1">
                    <h2 className="text-[22px] font-black text-slate-900 dark:text-white capitalize tracking-tight flex items-center gap-2">
                      {selectedVendor.shopName}
                      {selectedVendor.isActive && <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.6)]" />}
                    </h2>
                  ...
                  <div className="space-y-6">
                    ...
                    <Section title="Command Actions" icon={Zap}>
                      <div className="grid grid-cols-2 gap-2.5">
                        ...
                        />
                        <ActionButton
                          onClick={() => handleAction(selectedVendor._id, 'delete')}
                          icon={Trash2}
                          label="Delete"
                          color="red"
                        />
...
const Section = ({ title, icon: Icon, children }) => (
  <div className="space-y-4">
    <div className="flex items-center gap-3 px-1">
      ...
      <h3 className="text-[11px] font-black text-slate-900 dark:text-white capitalize tracking-[0.2em] opacity-90">{title}</h3>
    </div>
    <div className="p-5 bg-gradient-to-b from-slate-50 to-white dark:from-gray-900/60 dark:to-gray-900/20 rounded-[2rem] border border-slate-100 dark:border-gray-800/60 shadow-sm space-y-4">
      {children}
    </div>
  </div>
);
