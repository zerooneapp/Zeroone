...
const AdminNotificationList = () => {
    ...
    const getIcon = (type) => {
        switch (type) {
            case 'VENDOR_APPROVED': return <CheckCircle2 size={18} className="text-emerald-500 dark:text-emerald-400" />;
            case 'VENDOR_REJECTED': return <AlertCircle size={18} className="text-red-500 dark:text-red-400" />;
            case 'NEW_BOOKING': return <ShoppingBag size={18} className="text-primary dark:text-white" />;
            case 'SYSTEM_ALERT': return <AlertCircle size={18} className="text-orange-500 dark:text-orange-400" />;
            case 'BROADCAST': return <Send size={18} className="text-indigo-500 dark:text-indigo-400" />;
            default: return <Bell size={18} className="text-primary dark:text-white" />;
        }
    };

    return (
        <div className="min-h-screen bg-slate-50 dark:bg-gray-950 pb-20">
            {/* 🏙️ PREMIUM HEADER - STICKY & ENHANCED */}
            <header className="px-5 pt-6 pb-4 sticky top-[-2rem] md:top-[-2rem] bg-white/80 dark:bg-gray-950/80 backdrop-blur-2xl z-40 border-b border-slate-200/60 dark:border-gray-800 shadow-sm -mx-5 -mt-8 mb-8">
                <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-primary/10 dark:bg-primary/20 rounded-2xl flex items-center justify-center text-primary dark:text-white border border-primary/20 shadow-inner">
                            <Bell size={24} />
                        </div>
                        <div>
                            ...
                            <p className="text-[10px] font-black text-primary dark:text-white/60 uppercase tracking-[0.2em] mt-1.5 opacity-80">
                        ...
                        {isSelectionMode ? (
                            <>
                                <button 
                                    ...
                                    className="p-2.5 text-slate-500 dark:text-gray-400 font-black text-[10px] uppercase tracking-widest hover:text-primary dark:hover:text-white transition-colors"
                                >
