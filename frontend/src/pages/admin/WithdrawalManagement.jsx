    ...
    <div className="p-4 md:p-8 space-y-6 animate-in fade-in duration-500 bg-slate-50 dark:bg-gray-950 min-h-screen font-sans pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1">Review partner payouts</p>
        </div>
          ...
          <div className="flex bg-white dark:bg-gray-900 p-1 rounded-lg border border-slate-200 dark:border-gray-800">
            {['all', 'pending', 'approved', 'rejected'].map((s) => (
              <button
                key={s}
                onClick={() => setFilter(s)}
                className={`px-3 py-1 text-[9px] font-black uppercase rounded-md transition-all ${filter === s ? 'bg-[#00246b] text-white shadow-md shadow-[#00246b]/10' : 'text-slate-400 hover:text-slate-600'}`}
              >
                {s}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 gap-3">
        {loading ? (
          ...
          <div className="py-16 text-center bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-slate-200">
            ...
            <p className="text-slate-400 font-black uppercase tracking-widest text-[11px]">No requests found</p>
          </div>
        ) : (
          <>
              ...
              >
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                  ...
                  <div className="flex flex-col gap-2 min-w-[240px] md:border-l border-slate-100 dark:border-gray-800/60 md:pl-6">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      <span className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">Amount</span>
                      <span className="text-sm font-black text-slate-900 dark:text-white">₹{req.amount.toLocaleString()}</span>
                    </div>

                    {/* Payout Details Row */}
                    <div className="flex items-center justify-between gap-4">
                      <span className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">Payout Details</span>
                      {req.method === 'upi' ? (
                        <span className="text-[10px] font-black text-slate-600 dark:text-slate-300 flex items-center gap-1 bg-indigo-50/50 dark:bg-indigo-500/10 px-2 py-0.5 rounded-md">
                          <CreditCard size={10} className="text-indigo-500" /> {req.upiDetails?.upiId}
                        </span>
                      ) : (
                        <span className="text-[10px] font-black text-slate-600 dark:text-slate-300 flex items-center gap-1 bg-blue-50/50 dark:bg-blue-500/10 px-2 py-0.5 rounded-md leading-none">
                    ...
                    <div className="flex items-center justify-between gap-4">
                      {req.status === 'pending' ? (
                        <>
                          <span className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">Actions</span>
                          <div className="flex items-center gap-2">
                            <button 
                        ...
                        <>
                          <span className="text-[8px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-wider">Processed</span>
                          <span className="text-[9px] font-black text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-gray-800 px-2 py-0.5 rounded-md">
                ...
                {req.adminNotes && (
                  <div className="mt-2 p-2 bg-slate-50/50 dark:bg-gray-800/40 rounded-lg border border-slate-100 dark:border-gray-800">
                    <p className="text-[9px] font-bold text-slate-500 dark:text-slate-300 leading-relaxed truncate"><span className="font-black uppercase text-[8px] mr-1 dark:text-white">Admin:</span> {req.adminNotes}</p>
                  </div>
                )}
              </motion.div>
            ))}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            ...
            >
              <div className="p-4 bg-[#00246b] text-white text-center relative">
                ...
                <p className="text-[9px] font-black uppercase tracking-widest opacity-70">Platform rules</p>
              </div>
              <div className="p-5 space-y-5">
                <div>
                  <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Min Withdrawal Limit (₹)</label>
                  <div className="relative">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setSettingsModal(false)} className="flex-1 py-2.5 bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-slate-400 font-black text-[10px] uppercase rounded-xl">Cancel</button>
            ...
            >
              <div className={`p-4 flex items-center gap-3 ${processModal.type === 'approve' ? 'bg-emerald-500' : 'bg-red-500'} text-white`}>
                ...
                <div>
                  ...
                  <p className="text-[9px] font-black uppercase tracking-widest opacity-70">Review carefully</p>
                </div>
              </div>
              ...
              <div className="p-5 space-y-4">
                {processModal.type === 'reject' && (
                  <div>
                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Rejection Reason *</label>
                    <textarea 
                      required
                ...
                <div>
                  <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Admin Notes (Optional)</label>
                  <textarea 
                    value={adminNotes}
                ...
                <div className="flex gap-2 pt-2">
                  <button onClick={() => setProcessModal({ isOpen: false, type: 'approve', requestId: null })} className="flex-1 py-2.5 bg-slate-100 dark:bg-gray-800 text-slate-600 dark:text-slate-400 font-black text-[10px] uppercase rounded-xl">Cancel</button>
