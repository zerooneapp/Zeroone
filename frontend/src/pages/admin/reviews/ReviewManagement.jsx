    ...
    <div className="space-y-5 pb-20 animate-in fade-in duration-500">
      ...
      <div className="p-5 px-6 bg-white dark:bg-gray-900 rounded-2xl border border-slate-200/60 dark:border-gray-800 shadow-sm flex flex-col lg:flex-row lg:items-center justify-between gap-5 border-b-2 border-b-primary/20">
        <div className="space-y-1">
          ...
          <p className="text-[12px] font-black text-slate-500 capitalize tracking-[0.2em] opacity-90">Global Moderation & Feedback Integrity Hub</p>
        </div>
            ...
            <button
              ...
              className={cn(
                ...
                filters.rating === rating
                  ...
                  : 'bg-slate-50 dark:bg-gray-800 text-slate-500 border-slate-100 dark:border-gray-700 hover:bg-slate-50'
              )}
            >
              {rating}
            </button>
          ...
          >
            <div className="flex items-start justify-between relative z-10">
              <div className="flex gap-3">
                <div className="w-9 h-9 rounded-lg bg-slate-50 dark:bg-gray-800 flex items-center justify-center text-[13px] font-black text-slate-500 border border-slate-100 dark:border-gray-700 capitalize">
                  {review.user?.[0] || 'U'}
                </div>
                <div className="space-y-1 leading-none">
                  ...
                  <p className="text-[10px] font-black text-slate-500 capitalize tracking-widest opacity-90">
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <span className="text-[9px] font-black text-slate-500 capitalize tracking-tighter opacity-80">
            ...
            <div className="bg-slate-50 dark:bg-gray-800/40 p-3.5 rounded-xl border border-slate-100 dark:border-gray-800 relative group-hover:bg-white dark:group-hover:bg-gray-800 transition-all">
              <div className="absolute -top-2.5 left-3.5 bg-white dark:bg-gray-900 px-2 flex items-center gap-1.5 border border-slate-100 dark:border-gray-800 rounded-md py-0.5 shadow-sm">
                ...
                <span className="text-[9px] font-black text-slate-500 capitalize tracking-widest opacity-90">Node Trace</span>
              </div>
            ...
            <div className="flex items-center gap-2 mt-auto pt-1">
              <div className="flex-1 h-9 flex items-center gap-2 px-3 opacity-100">
                <CheckCircle2 size={14} className="text-emerald-500" />
                <span className="text-[10px] font-black text-slate-500 capitalize tracking-widest">Live on Platform</span>
              </div>
              <button
                onClick={() => handleAction(review._id, 'delete')}
                className="w-9 h-9 text-slate-500 hover:text-rose-500 transition-all flex items-center justify-center"
              >
            ...
            <div className="flex items-center gap-1.5">
              {[...Array(totalPages)].map((_, i) => (
                <button
                  ...
                  className={`w-9 h-9 rounded-xl font-black text-[11px] uppercase tracking-wider transition-all ${currentPage === i + 1
                      ? 'bg-slate-900 text-white shadow-md shadow-slate-900/10'
                      : 'bg-white dark:bg-gray-800 text-slate-500 border border-slate-100 dark:border-gray-700 hover:bg-slate-50'
                    }`}
                >
                  {i + 1}
                </button>
      ...
      {loading && (
        <div className="p-20 text-center space-y-4 bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-slate-200 dark:border-gray-800 animate-pulse shadow-sm">
          <div className="w-16 h-16 bg-slate-50 dark:bg-gray-800 rounded-xl flex items-center justify-center mx-auto text-slate-500 border border-slate-100">
            <ShieldAlert size={40} strokeWidth={3} />
          </div>
          <p className="font-black text-slate-500 capitalize tracking-widest text-[12px] opacity-80">Loading moderation feed</p>
        </div>
      )}
      ...
      {!loading && data.reviews.length === 0 && (
        <div className="p-20 text-center space-y-4 bg-white dark:bg-gray-900 rounded-2xl border border-dashed border-slate-200 dark:border-gray-800 animate-in zoom-in duration-500 shadow-sm">
          ...
          <p className="font-black text-slate-500 capitalize tracking-widest text-[12px] opacity-80">No reputation nodes detected in this quadrant</p>
        </div>
      )}
    </div>
  );
...
const SummaryCard = ({ label, value, icon: Icon, color, sub }) => (
  <div className="p-5 bg-white dark:bg-gray-900 rounded-2xl border border-slate-200/60 dark:border-gray-800 shadow-sm space-y-4 transition-all hover:shadow-md border-b-2" style={{ borderBottomColor: color === 'primary' ? 'var(--primary)' : color === 'rose' ? '#f43f5e' : color === 'amber' ? '#f59e0b' : '#3D3A3A' }}>
    ...
    <div className="pt-3 border-t border-slate-50 dark:border-gray-800 flex items-center gap-2">
      ...
      <p className="text-[11px] font-black text-slate-500 capitalize tracking-tighter opacity-90">{sub}</p>
    </div>
  </div>
);
