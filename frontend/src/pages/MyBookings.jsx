          ...
          {['upcoming', 'completed', 'cancelled'].map(tab => {
            ...
            return (
              <button
                ...
                className={`flex-1 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border ${isActive
                    ? 'bg-slate-900 border-slate-900 text-white shadow-lg shadow-black/10'
                    : 'bg-white dark:bg-gray-900 border-slate-200/60 dark:border-gray-800 text-slate-400'
      ...
      <main className="px-4 mt-5">
        {filteredBookings.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-400 text-xs font-black uppercase tracking-widest">No {activeTab} bookings</p>
            <button
              className="mt-6 px-6 py-2.5 bg-primary text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 active:scale-95 transition-all"
              onClick={() => navigate('/')}
            >
              Book a Service
            </button>
          ...
          <div className="space-y-3.5">
            {filteredBookings.map((booking) => (
              <div
                key={booking._id}
                className="bg-white dark:bg-gray-900 p-3 rounded-2xl border border-slate-200/60 dark:border-gray-800 shadow-sm transition-all active:scale-[0.98]"
                onClick={() => navigate(`/booking-status/${booking._id}`)}
              >
                <div className="flex justify-between items-start mb-1.5 leading-none">
                  <div>
                    <h3 className="font-extrabold text-[13px] text-gray-900 dark:text-white truncate max-w-[200px] uppercase tracking-tight">{booking.vendorId.shopName}</h3>
                  ...
                  <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border leading-none shadow-sm ${getStatusColor(booking.status)}`}>
                    {booking.status}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 py-2.5 border-y border-dashed border-slate-100 dark:border-gray-800/50 my-2.5">
                  ...
                  <div className="space-y-1 text-right flex flex-col items-end">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none">Time</p>
                ...
                <div className="flex items-center justify-between leading-none mt-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-black uppercase text-slate-400 tracking-widest">{booking.staffId?.name || 'Auto Assigned'}</span>
                  </div>
