...
import { ArrowLeft, Plus, Crown, Calendar, Info, Users, Phone, ShieldCheck, Clock, CheckCircle2, XCircle, Trash2, AlertTriangle, Share2, Search, X } from 'lucide-react';
...
const MembershipPlans = () => {
  ...
  const shareOnWhatsApp = (client, plan) => {
    const services = plan.services?.map(
      s => `• ${s.serviceId?.name || 'Service'} (${s.usageLimit} uses)`
    ).join('\n') || 'No services listed';
    ...
    const message =
      ...
      `Join our exclusive membership and enjoy premium services! 🎉`;
  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark pb-32">
      ...
      {/* 📤 SHARE PLAN — CLIENT PICKER BOTTOM SHEET */}
      <AnimatePresence>
        {sharingPlan && (
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-slate-100 dark:border-gray-800">
                <div>
                  ...
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{sharingPlan.name} Plan</p>
                </div>
                <button
                  onClick={() => setSharingPlan(null)}
                ...
                {(() => {
                  ...
                  return filtered.map(client => (
                    <button
                      ...
                      onClick={() => shareOnWhatsApp(client, sharingPlan)}
